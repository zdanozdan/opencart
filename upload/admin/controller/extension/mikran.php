...
class ControllerExtensionMikran extends Controller { 
	private $error = array();
	private $ssl = 'SSL';
    ...
    public function translate($param=false) {
        $this->load->model('localisation/language');
        $this->load->model('extension/mikran');

        $param = explode(":",$param);
        $lang = (isset($param[0])  ?  $param[1] : false);

        if(sizeof(explode('-',$param[1])) < 2) {
            $languages = $this->model_localisation_language->getLanguages();
            print_r($languages);
            print_r(array("Error ! Please put full language code like en-gb or de-de. Check available codes above"));
            return;
        }

        if ($param[0] == 'category') {
            $this->model_extension_mikran->translateCategories($lang);
            return;
        }

        if ($param[0] == 'product') {
            $this->model_extension_mikran->translateProducts($lang);
            return;
        }

        if ($param[0] == 'option') {
            $this->model_extension_mikran->translateOptions();
            return;
        }

        print_r(array("Specify translation option: product,category and target language, for example translate product:de-de"));
    }

    public function uninstall() {
        $this->load->model('setting/event');
        $this->model_setting_event->deleteEventByCode('mikran_product_breadcrumbs_handler');

        $retval = array('uninstall mikran events');
        $retval['mikran_product_breadcrumbs_handler']='product/product/addProductBreadcrumbsHandlerBefore';
    ...
    public function install() {
        $this->load->model('setting/event');
        $this->model_setting_event->addEvent('mikran_product_breadcrumbs_handler','catalog/view/product/product/before','product/product/addProductBreadcrumbsHandlerBefore');

        print_r(array('mikran_product_breadcrumbs_handler'=>'product/product/addProductBreadcrumbsHandlerBefore'));
    }
    ...
    public function locale() {
        ...
        $message=array('Loading Polish locales - currencies,taxes and statuses translation');        

        $lang_id = $this->model_extension_mikran->addLangPolish();
        $message[] = 'loaded PL lang';
        $this->model_extension_mikran->addCurrencyPLN();
        $message[] = 'loaded PLN';
        #print_r($this->model_extension_mikran->getCurrencies());
        $this->model_extension_mikran->loadStatuses($lang_id);
        $message[] = 'loaded order and other statuses';
        $geo_zone_id = $this->model_extension_mikran->loadVATZone();
        $message[] = 'PL geo zone';        
        $this->model_extension_mikran->loadVATTaxesPl($geo_zone_id);
        $message[] = 'loaded VAT taxes';
        $this->model_extension_mikran->addLengthWeigthPl($lang_id);
        $message[] = 'loaded length and width defintions';        
        print_r($message);
    }
    ...
    public function options($param = False) {
        ...
        if ($param == 'load') {
            $this->model_extension_mikran->loadMikranOptions();
            return;
        }

        if ($param == 'delete') {
            $this->model_extension_mikran->deleteAllOptions();
            return;
        }

        print PHP_EOL."Usage : load,delete".PHP_EOL;
    }

    public function images($param = False) {
        $this->load->language('extension/mikran');
		$this->document->setTitle($this->language->get('heading_title'));
		$this->load->model('extension/mikran');

        if ($param == 'load') {
            $this->model_extension_mikran->loadMikranImages();
            return;
        }

        if ($param == 'delete') {
            $this->model_extension_mikran->deleteMikranImages();
            return;
        }
    ...
    public function products($param = False) {
        $this->load->language('extension/mikran');
		$this->document->setTitle($this->language->get('heading_title'));
		$this->load->model('extension/mikran');

        if ($param == 'load') {
            $this->model_extension_mikran->loadMikranProducts();
            return;
        }

        if ($param == 'delete') {
            $this->model_extension_mikran->deleteMikranProducts();
            return;
        }
    ...
    public function categories() {
		...
		$this->document->setTitle($this->language->get('heading_title'));
		$this->load->model('extension/mikran');
        $this->model_extension_mikran->removeAllCategories();
        $this->model_extension_mikran->loadMikranCategories();
    }
