...
class ModelToolMikran extends Model {
    ...
    public function getCurrencies() {
        $currencies_query = $this->db->query("SELECT code FROM " . DB_PREFIX . "currency");
        return $currencies_query->rows;
    }

    public function loadStatuses($language_id) {
        ...
        if (count($stock_statuses_query) && $language_id) {
            ...
            foreach ($stock_statuses_query as $result) {		
                if(isset($stock_statuses[$result['name']]))
                    {
                        $this->db->query("UPDATE " . DB_PREFIX . "stock_status SET name = '" . $this->db->escape($stock_statuses[$result['name']]) . "' WHERE stock_status_id = '" . (int)$result['stock_status_id'] . "' AND language_id = '" . (int)$language_id . "'");
                    }		
            }
        }
        ...
        if (count($order_statuses_query) && $language_id) {
            ...
            foreach ($order_statuses_query as $result) {		
                if(isset($order_statuses[$result['name']]))
                    {
                        $this->db->query("UPDATE " . DB_PREFIX . "order_status SET name = '" . $this->db->escape($order_statuses[$result['name']]) . "' WHERE order_status_id = '" . (int)$result['order_status_id'] . "' AND language_id = '" . (int)$language_id . "'");
                    }		
            }
        }
        ...
        if (count($return_statuses_query) && $language_id) {
            ...
            foreach ($return_statuses_query as $result) {		
                if(isset($return_statuses[$result['name']]))
                    {
                        $this->db->query("UPDATE " . DB_PREFIX . "return_status SET name = '" . $this->db->escape($return_statuses[$result['name']]) . "' WHERE return_status_id = '" . (int)$result['return_status_id'] . "' AND language_id = '" . (int)$language_id . "'");
                    }		
            }
        }
        ...
        if (count($return_action_statuses_query) && $language_id) {
            ...
            foreach ($return_action_statuses_query as $result) {		
                if(isset($return_action_statuses[$result['name']]))
                    {
                        $this->db->query("UPDATE " . DB_PREFIX . "return_action SET name = '" . $this->db->escape($return_action_statuses[$result['name']]) . "' WHERE return_action_id = '" . (int)$result['return_action_id'] . "' AND language_id = '" . (int)$language_id . "'");
                    }		
            }
        }
		...
		if(isset($return_reason_statuses[$result['name']]))
		{
		    $this->db->query("UPDATE " . DB_PREFIX . "return_reason SET name = '" . $this->db->escape($return_reason_statuses[$result['name']]) . "' WHERE return_reason_id = '" . (int)$result['return_reason_id'] . "' AND language_id = '" . (int)$language_id . "'");
		}		
	    }
	}
    }

    public function loadVATZone() {
	$this->load->model('localisation/geo_zone');
	$geo_zone_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "geo_zone WHERE name = 'PL VAT'");
	
	if($geo_zone_query->num_rows == 0)
	{
	    $this->model_localisation_geo_zone->addGeoZone(array(
		'name' => 'PL VAT',
    ...
    public function loadVATTaxesPl($geo_zone_id) {

	// Add tax rates	
	$this->load->model('localisation/tax_rate');
	
	$tax_rate_23_query = $this->db->query("SELECT tax_rate_id FROM " . DB_PREFIX . "tax_rate WHERE geo_zone_id = '" . (int)$geo_zone_id . "' AND name = 'VAT23'");
	
	if ($tax_rate_23_query->num_rows == 0)
	{
	    $this->model_localisation_tax_rate->addTaxRate(array(
		'name' 						=> 'VAT23',
		'rate' 						=> 23.0,
		'type'  					=> 'P',
	...
	$tax_rate_8_query = $this->db->query("SELECT tax_rate_id FROM " . DB_PREFIX . "tax_rate WHERE geo_zone_id = '" . (int)$geo_zone_id . "' AND name = 'VAT8'");
	...
	$tax_rate_5_query = $this->db->query("SELECT tax_rate_id FROM " . DB_PREFIX . "tax_rate WHERE geo_zone_id = '" . (int)$geo_zone_id . "' AND name = 'VAT5'");
	...
	$tax_class_23_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "tax_class WHERE title = 'VAT23'");
	
	if($tax_class_23_query->num_rows == 0) {
	    
	    $this->model_localisation_tax_class->addTaxClass(array(
		'title' 		=> 'VAT23',
		'description' 	=> 'Podatek VAT23',
		'tax_rule' 		=> array(
		    array(
	...
	$tax_class_8_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "tax_class WHERE title = 'VAT8'");
	
	if($tax_class_8_query->num_rows == 0) {
	    
	    $this->model_localisation_tax_class->addTaxClass(array(
		'title' 		=> 'VAT8',
		'description' 	=> 'Podatek VAT8',
		'tax_rule' 		=> array(
		    array(
	...
	$tax_class_5_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "tax_class WHERE title = 'VAT5'");
	
	if($tax_class_5_query->num_rows == 0) {
		
	    $this->model_localisation_tax_class->addTaxClass(array(
		'title' 		=> 'VAT5',
		'description' 	=> 'Podatek VAT5',
		'tax_rule' 		=> array(
		    array(
    ...
    public function addLengthWeigthPl($language_id) {
	// Edit length class
	...
	if (count($length_class_query) && $language_id) {
		...
		if(isset($length_class[$result['title']]))
		{
		    $this->db->query("UPDATE " . DB_PREFIX . "length_class_description SET title = '" . $this->db->escape($length_class[$result['title']]) . "' WHERE length_class_id = '" . (int)$result['length_class_id'] . "' AND language_id = '" . (int)$language_id . "'");
		}		
	    }
	}
	...
	if (count($weight_class_query) && $language_id) {
		...
		if(isset($weight_class[$result['title']]))
		{
		    $this->db->query("UPDATE " . DB_PREFIX . "weight_class_description SET title = '" . $this->db->escape($weight_class[$result['title']]) . "' WHERE weight_class_id = '" . (int)$result['weight_class_id'] . "' AND language_id = '" . (int)$language_id . "'");
		}		
	    }
	}
    }
    
    public function addLangPolish() {
        $this->load->model('localisation/language');

        $languages_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "language WHERE code = 'pl-pl'");

        if ($languages_query->num_rows == 0) {
            $language_id = $this->model_localisation_language->addLanguage(array(
                'name'      	=> 'polski',
                'code' 			=> 'pl-pl',
                'sort_order'	=> '1',
                'status' 		=> '1'
    ...
    public function addCurrencyPLN() {
        ...
        return $currencies_query->num_rows;
    }
    ...
    protected function getMikranConn() {
        $server = "localhost";
        $port = "3306";
        $login = "root";
        $pass = "zdanek123";
        ...
        try {
            $conn = new PDO("mysql:host=$server;dbname=mikran_1;charset=utf8", $login, $pass);
            return $conn;
        }
    ...
    protected function find_tax($rate,$tax_classes) {        
        foreach($tax_classes->rows as $tax_class) {
            if ($tax_class['title'] == 'VAT'.$rate) {
                return $tax_class['tax_class_id'];
            }
        }
    }
    ...
    public function loadMikranOptionsSql($sql) {
        $this->clearCache();
        ...
        foreach ($results as $result) {
            $options = $result['xml_options'];
            $options = explode("\n", $options);

            if (isset($options[0]) && trim($options[0]) == 'multi') {
                ...
                foreach ($options as $key => $option) {
                    ...
                    $v = trim($option);
                    if (empty($v) && isset($options[$key + 1])) {
                        $comb = trim($options[$key + 1]);
                        $skip = $key + 1;
                        continue;
                    } else if (!empty($v)) {
                        $combinations[$comb][] = trim($v).' #_'.$result['id'];
                        $a1 = isset($product_combinations[$comb][trim($v)]) == true ? $product_combinations[$comb][trim($v)] : array();
                        $product_combinations[$comb][trim($v)] = array_merge($a1,array($result['id']));
                    }
                }
            }
        }
        ...
        $unique = array();
        foreach($product_combinations as $key=>$value) {
            array_multisort(array_keys($value), SORT_NATURAL, $value);
            $unique[ucfirst($key)] =$value;
        }

        
        foreach($unique as $key=>$value) {
            $option = array();
            $q = $this->db->query("SHOW TABLE STATUS LIKE 'oc_option'");
            $option_id = $q->row['Auto_increment'] + 1;

            $option['option_id'] = $option_id;


            $radio = true;
            foreach($value as $option_name=>$product_keys) {
                $im = (explode(",",$option_name));
                if (sizeof($im) < 3) {
                  $radio = false;
                }
            }

            $option['type'] = ($radio==true) ? 'radio':'select';
            
            $option['sort_order'] = 0;
            
            $option['names'] = array('en-gb'=>$key,'pl-pl'=>$key);

            $this->storeOptionIntoDatabase( $option, $languages );

            $q = $this->db->query("SHOW TABLE STATUS LIKE 'oc_option_value'");
            $option_value_id = $q->row['Auto_increment'] + 1;

            foreach($value as $option_name=>$product_ids) {
                $exist_image = false;
                $option_value = array();
                #$vals = explode("#_",$result);

                $images = explode(",",$option_name);
                if (sizeof($images) == 3) {
                    $image = 'catalog/mikran/product/'.trim($images[2]);
                ...
                $option_value['sort_order'] = 0;
                $option_value['names'] = array('en-gb'=>trim($images[0]),'pl-pl'=>trim($images[0]));
                ...
                $product_option_value['option_id'] = $option['option_id'];
                ...
                $product_option_value['product_option_id'] = $product_option_ids[$option['option_id']];
    ...
    public function deleteMikranImages() {
        $this->deleteAdditionalImages();
    }

    public function loadMikranImages() {

        #d = {"zd_Id":row.zd_Id, "zd_IdTowar":row.zd_IdTowar, "zd_Glowne":row.zd_Glowne, "zd_Nazwa":row.zd_Nazwa,'zd_Aktywne':row.zd_Aktywne,'zd_Path':IMAGE_PATH+filename,'pulled':pull,'pulled_date':datetime.datetime.now()}
        #conn.hmset("images:"+str(row.zd_Id), d)
        $redis = new Redis();
        $redis->connect('127.0.0.1');
        $query = $this->db->query( "SELECT product_id FROM `".DB_PREFIX."product`" );

        foreach($query->rows as $result) {
            $ranges = $redis->zRangeByScore('images:zd_IdTowar', $result['product_id'], $result['product_id']);
            foreach ($ranges as $range) {
                $values = $redis->hmget($range, array("zd_Id", 'zd_Glowne', "zd_IdTowar",'zd_Nazwa','zd_Aktywne','zd_Path','pulled','pulled_date'));
                if ($values['zd_Glowne'] == "False") {
                    $image = array();
                    $image['product_id'] = $values['zd_IdTowar'];
                    $image['image_name'] = 'catalog/mikran/product/'.basename($values['zd_Path']);
                    $image['sort_order'] = 0;
                    $old_product_image_ids = array();
                    $exist_sort_order = true;
    ...
    public function deleteMikranProducts() {
        ...
        $sql = 'SELECT m.*, d.num as quantity FROM main m LEFT JOIN depository d on (m.user_id = d.user_id_main)';

        $results = $conn->query($sql);
        foreach ($results as $result) {
            printf("Deleting product with id: %d\r\n",$result['id']);
            $this->deleteProduct( $result['id'], $exist_table_product_tag );
        }
    }

    public function loadMikranProducts() {
        $conn = $this->getMikranConn();

		$exist_table_product_tag = false;

        $exist_meta_title = true;
...
#        $sql = "select * from main left join depository d on main.user_id = d.user_id_main where main.active=1 and main.id=1";

        $sql = 'SELECT m.*, d.num as quantity FROM main m LEFT JOIN depository d on (m.user_id = d.user_id_main) where m.active=1';
        ...
        foreach ($results as $result) {
            ...
            $names = array('en-gb'=>$result['name_L1'],'pl-pl'=>$result['name_L0']);
			$product['names'] = $names;
			$categories = $result['id_category1'];
            if ($result['id_category2'] > 0) {
                $categories = $result['id_category2'];
            }
            if ($result['id_category3'] > 0) {
                $categories = $result['id_category3'];
            }
            if ($result['id_category4'] > 0) {
                $categories = $result['id_category4'];
            }
            if ($result['id_category5'] > 0) {
                $categories = $result['id_category5'];
            }
			...
			$product['model'] = $result['id'];
			$product['manufacturer_name'] = $result['producer'];
			$product['image'] = 'catalog/mikran/product/'.$result['photo'];
			$product['shipping'] = 'YES';
            $price = $result['vat'] > 0 ? $result['price_brutto'] / (1 + $result['vat']/100):$result['price_brutto'];
            $product['price_brutto'] = $price;
            $product['vat'] = $result['vat'];
			$product['price'] = $price;
			$product['points'] = $result['points_value'];
			$product['date_added'] = 'NOW()';
			$product['date_modified'] = $result['date_update'];
			$product['date_available'] = 'NOW()';
			$product['weight'] = 0;
			$product['weight_unit'] = $default_weight_unit;
			$product['status'] = "YES";

            #disable products with no name
            if(strlen($result['name_L0']) == 0) {
                $product['status'] = 'NO';
            }
            
            #0,8,23
            $tax_class_id = $this->find_tax($result['vat'],$tax_classes);
            if ($tax_class_id) {
            ...
            $descriptions = array('en-gb'=>$result['xml_description_L0'],'pl-pl'=>$result['xml_description_L0']);
			$product['descriptions'] = $descriptions;

            $default_stock_status_id = 5;
            $product['stock_status_id'] = $default_stock_status_id;
            
            $title_pl = $this->config->get('config_name').' - sklep - '.$result['name_L0'];
            $title_en = strlen($result['name_L1']) > 0 ? $this->config->get('config_name').' - shop - '.$result['name_L1']:$this->config->get('config_name').'- shop -'.$result['name_L0'];
            $meta_titles = array('en-gb'=>$title_en,'pl-pl'=>$title_pl);
            
			if ($exist_meta_title) {
				$product['meta_titles'] = $meta_titles;
			}

            $meta_descriptions = array('en-gb'=>strip_tags($result['xml_description_L0']),'pl-pl'=>strip_tags($result['xml_short_description_L0']));
			...
			#}
			$product['measurement_unit'] = $default_measurement_unit;
			$product['sku'] = 'mik_'.$result['id'];
			$product['upc'] = $result['id'];
			if (in_array('ean',$product_fields)) {
				$product['ean'] = $result['id'];
			}
			if (in_array('jan',$product_fields)) {
				$product['jan'] = $result['id'];
			}
	...
	#		}
			...
			$this->storeProductIntoDatabase( $product, $languages, $product_fields, $exist_table_product_tag, $exist_meta_title, $layout_ids, $available_store_ids, $manufacturers, $weight_class_ids, $length_class_ids, $url_alias_ids );
                ...
                echo 'Exeption no '.$e->getCode().', message:'.$e->getMessage();
            }
		}
    }
    ...
    public function translateOptions() {
        $source_language_id = $this->getDefaultLanguageId('pl-pl');
        $languages = $this->getLanguages();

        $options = $this->getOptions($languages);
        foreach($options as $count=>$option) {
            $source_txt = $option['name']['pl-pl'];
            foreach($option['name'] as $code=>$option_name) {
                if($code != 'pl-pl') {
                    $options[$count]['name'][$code] = $this->translate($source_txt,explode("-",$code)[0]);
                }
            }
        }
        ...
        foreach($options as $option) {
            ...
            foreach ($languages as $language) {
                ...
                $sql = "UPDATE " . DB_PREFIX . "option_description SET name = '" . $name . "' WHERE option_id = '" . $option_id . "' AND language_id = '" . (int)$language_id . "'";
                $this->db->query( $sql );
            }
        }
        ...
        $languages = $this->getLanguages();
        $option_values = $this->getOptionValues( $languages );

        foreach($option_values as $count=>$option_value) {
            $source_txt = $option_value['name']['pl-pl'];
            foreach($option_value['name'] as $code=>$option_name) {
                if($code != 'pl-pl') {
                    $option_values[$count]['name'][$code] = $this->translate($source_txt,explode("-",$code)[0]);
                }
            }
        }
        ...
        foreach($option_values as $option_value) {
            ...
            $names = $option_value['name'];

            foreach ($languages as $language) {
                $language_code = $language['code'];
                $language_id = $language['language_id'];
                $name = isset($names[$language_code]) ? $this->db->escape($names[$language_code]) : '';
                $sql = "UPDATE " . DB_PREFIX . "option_value_description SET name = '" . $name . "' WHERE option_value_id = '" . $option_value_id . "' AND language_id = '" . (int)$language_id . "'";
                $this->db->query( $sql );
            }
        }
    }

    public function translateProducts($target_code=false) {
        //$source_language_id = $this->getDefaultLanguageId('pl-pl');
        ...
        if($target_code) {
            foreach($languages as $count=>$language) {
                if(in_array($language['code'],$codes) == false) {
                    unset($languages[$count]);
                }
            }
        }

        $product_descriptions = $this->getSimpleProductDescriptions( $languages );
        //$product_descriptions = array('1'=>$product_descriptions[1]);
        //print_r($product_descriptions);
        ...
        foreach($product_descriptions as $product_id=>$description) {
            $name_pl = $description['pl-pl']['name'];
            $tags_pl = $description['pl-pl']['tag'];
            
            foreach($description as $code=>$lang_description) {
                $language_id = $lang_description['language_id'];
                $target = explode('-',$code)[0];
                
                $name = $this->translate($name_pl,explode('-',$code)[0]);
                $product_descriptions[$product_id][$code]['name'] = $name;
                
                print_r(array($name_pl=>$code.":".$name));
                $product_descriptions[$product_id][$code]['name'] = $name;
                
                $shop_title = $this->translate('sklep',explode('-',$code)[0]);
                $product_descriptions[$product_id][$code]['meta_title'] = $this->config->get('config_name').' - '.$shop_title.' - '.$name;
                $product_descriptions[$product_id][$code]['meta_keyword'] = $name;
                
                $product_cat = $this->model_catalog_product->getProductCategories($product_id);
                if(isset($product_cat[0])) {
                    ...
                    foreach($path as $p) {
                        $category_names = $this->model_catalog_category->getCategoryDescriptions($p['path_id']);
                        $product_descriptions[$product_id][$code]['meta_keyword'] .= ','. $category_names[$language_id]['name'];
                    }
                }
                
                //translate tags
                $tags = explode(",",$tags_pl);
                $translated_tags = array();
                foreach($tags as $tag) {
                    $translated_tags[] = $this->translate($tag,explode('-',$code)[0]);
                }

                $product_descriptions[$product_id][$code]['tag'] = implode(",",$translated_tags);
        ...
        foreach($product_descriptions as $product_id=>$description) {
            foreach($description as $code=>$lang_description) {
                $name = $lang_description['name'];
                $meta_title = $lang_description['meta_title'];
                $meta_keyword = $lang_description['meta_keyword'];                
                $language_id = $lang_description['language_id'];
                $tag = $lang_description['tag'];
                
                //TODO
                $meta_description = "";
                $description = "";
                
                $sql = "UPDATE " . DB_PREFIX . "product_description SET name = '" . $name . "' , meta_title = '".$meta_title."' , meta_keyword = '".$meta_keyword."' , tag = '".$tag."' WHERE product_id = '" . $product_id . "' AND language_id = '" . (int)$language_id . "'";
                $this->db->query($sql);
                print_r(array($sql));
            }
        }            
    ...
    public function translateCategories($target_code = false) {
        $source_language_id = $this->getDefaultLanguageId('pl-pl');
        ...
        $categories = $this->getCategoriesDescription($languages);
        #TEST
        //$categories = array('12534'=>$categories[12534]);

        //Autogenerate title keywords in format: cateogories,name
        $this->load->model('catalog/category');
        
        foreach($categories as $category_id=>$description) {
            ...
            foreach($description as $code=>$lang_description) {
                ...
                $name = $this->translate($name_pl,explode('-',$code)[0]);
                $categories[$category_id][$code]['name'] = $name;                
            }
        }
        ...
        foreach($categories as $category_id=>$description) {
            foreach($description as $code=>$lang_description) {
                ...
                $sql  = "UPDATE " . DB_PREFIX . "category_description SET name = '" . $name . "' WHERE category_id = '" . $category_id . "' AND language_id = '" . (int)$language_id . "'";
                $this->db->query($sql);
                print_r(array($sql));
            }
        }
        ...
        foreach($categories as $category_id=>$description) {
            foreach($description as $code=>$lang_description) {
                ...
                $categories[$category_id][$code]['meta_title'] = $this->config->get('config_name') . ' - '.$c.': ';
                $categories[$category_id][$code]['meta_description'] = $this->config->get('config_name') . ' - '.$c.': ';
                ...
                $categories[$category_id][$code]['meta_title'] .= implode(" > ",$category_names);
                $categories[$category_id][$code]['meta_description'] .= implode(" > ",$category_names);
                $categories[$category_id][$code]['meta_keyword'] = implode(",",$category_names);
            }
        }
        ...
        foreach($categories as $category_id=>$description) {
            foreach($description as $code=>$lang_description) {
                $meta_title = $lang_description['meta_title'];
                $meta_description = $lang_description['meta_description'];
                $meta_keyword = $lang_description['meta_keyword'];
                $language_id = $lang_description['language_id'];

                $sql  = "UPDATE " . DB_PREFIX . "category_description SET meta_title = '" . $meta_title . "', meta_description= '" . $meta_description . "', meta_keyword = '" . $meta_keyword ."' WHERE category_id = '" . $category_id . "' AND language_id = '" . (int)$language_id . "'";
                $this->db->query($sql);
                print_r(array($sql));
            }
        }
    ...
    public function loadMikranCategories() {
        $conn = $this->getMikranConn();
        ...
        $store_ids = $this->getAvailableStoreIds()[0];
        $layout = "";
        $status = "YES";

        $dictionary = array();
        $dsql = "select id,wordbase,en from dictionary where en is not null";
        $msg = sprintf('Prepare dictionary query', $dsql);
        $results = $conn->query($dsql);
        foreach($results as $result) {
            $dictionary[$result['wordbase']]=$result['en'];
        }
        ...
        foreach ($results as $result) {
            ...
            if ($result['id_category1'] != 0 && strlen($result['category1']) > 0) {
                $name_en = isset($dictionary[$result['category1']]) ? $dictionary[$result['category1']] : $result['category1'];
                $path_pl[] = $result['category1'];
                if(isset($dictionary[$result['category1']])) {
                    $trans = $dictionary[$result['category1']];
                    $path_en[] = $trans;
                } else {
                    $paths_en[] = $result['category1'];
                }
                array_push($sote_cats,array('parent_id'=>0,'level'=>0,'id'=>$result['id_category1'],'name'=>$result['category1'],'name_en'=>$name_en,'path_pl'=>$path_pl,'path_en'=>$path_en));
            }

            if ($result['id_category2'] != 0 && strlen($result['category2']) > 0) {
                $name_en = isset($dictionary[$result['category2']]) ? $dictionary[$result['category2']] : $result['category2'];
                $path_pl[] = $result['category2'];
                if(isset($dictionary[$result['category2']])) {
                    $trans = $dictionary[$result['category2']];
                    $path_en[] = $trans;
                } else {
                    $paths_en[] = $result['category2'];
                }
                array_push($sote_cats,array('parent_id'=>$result['id_category1'],'level'=>1,'id'=>$result['id_category2'],'name'=>$result['category2'],'name_en'=>$name_en,'path_pl'=>$path_pl,'path_en'=>$path_en));
            }

            if ($result['id_category3'] != 0 && strlen($result['category3']) > 0) {
                $name_en = isset($dictionary[$result['category3']]) ? $dictionary[$result['category3']] : $result['category3'];
                $path_pl[] = $result['category3'];
                if(isset($dictionary[$result['category3']])) {
                    $trans = $dictionary[$result['category3']];
                    $path_en[] = $trans;
                } else {
                    $paths_en[] = $result['category3'];
                }
                array_push($sote_cats,array('parent_id'=>$result['id_category2'],'level'=>2,'id'=>$result['id_category3'],'name'=>$result['category3'],'name_en'=>$name_en,'path_pl'=>$path_pl,'path_en'=>$path_en));
            }
            
            if ($result['id_category4'] != 0 && strlen($result['category4']) > 0) {
                $path_pl[] = $result['category4'];
                if(isset($dictionary[$result['category4']])) {
                    $trans = $dictionary[$result['category4']];
                    $path_en[] = $trans;
                } else {
                    $paths_en[] = $result['category4'];
                }
                $name_en = isset($dictionary[$result['category4']]) ? $dictionary[$result['category4']] : $result['category4'];
                array_push($sote_cats,array('parent_id'=>$result['id_category3'],'level'=>3,'id'=>$result['id_category4'],'name'=>$result['category4'],'name_en'=>$name_en,'path_pl'=>$path_pl,'path_en'=>$path_en));
            }

            if ($result['id_category5'] != 0 && strlen($result['category5']) > 0) {
                $name_en = isset($dictionary[$result['category5']]) ? $dictionary[$result['category5']] : $result['category5'];
                $path_pl[] = $result['category5'];
                if(isset($dictionary[$result['categor5']])) {
                    $trans = $dictionary[$result['category5']];
                    $path_en[] = $trans;
                } else {
                    $paths_en[] = $result['category5'];
                }
                array_push($sote_cats,array('parent_id'=>$result['id_category4'],'level'=>4,'id'=>$result['id_category5'],'name'=>$result['category5'],'name_en'=>$name_en,'path_pl'=>$path_pl,'path_en'=>$path_en));
            }

            foreach($sote_cats as $cat) {
                $category = array();
                ...
                $names = array('en-gb'=>$cat['name_en'],'pl-pl'=>$cat['name']);
                ...
                $descriptions = array();
                #$descriptions = array('en-gb'=>$cat['name_en'],'pl-pl'=>$cat['name']);
            
                $category['descriptions'] = $descriptions;

                $category_pl = join(' > ',$cat['path_pl']);
                $category_en = join(' > ',$cat['path_en']);

                $exist_meta_title = true;
                if ($exist_meta_title) {
                    $title_pl = $this->config->get('config_name') . ' - Kategoria: ' . $category_pl;
                    $title_en = $this->config->get('config_name') . ' - Category: ' . $category_en;
                    $category['meta_titles'] = array('en-gb'=>$title_en,'pl-pl'=>$title_pl);
                }

                $meta_descriptions = array('en-gb'=>"Category: ".$category_en,'pl-pl'=>"Kategoria: ".$category_pl);
                $category['meta_descriptions'] = $meta_descriptions;

                $meta_keywords = array(
                    'en-gb'=>join(' ', $cat['path_en']),
                    'pl-pl'=>join(' ', $cat['path_pl']),
                );
                ...
                try {
                    ...
                    echo "Created new category: ".$cat['name'] .' : '.$cat['name_en'].PHP_EOL;
                }
                catch (Exception $e) {
                    printf("Exeption on %s:%s\r\n",$category_pl,$category_en);
                }
            }
        }
    }
    ...
    public function loadtest() {
        ...
        $status = "YES";
        $category = array();
        $category['category_id'] = "669";
        $category['image'] = "";
        $category['parent_id'] = 668;
        $category['sort_order'] = 3;
        $category['date_added'] = "NOW()";
        $category['date_modified'] = "NOW()";

        $names = array('en-gb'=>'sub1','pl-pl'=>'sub1');

        $category['names'] = $names;
        $category['top'] = "1";
        $category['columns'] = 1;

        $descriptions = array('en-gb'=>'ENG description','pl-pl'=>"POL opis");
        ...
        if ($exist_meta_title) {
            $meta_titles = array('en-gb'=>'ENG meta tile description','pl-pl'=>"POL meta title opis");
            $category['meta_titles'] = $meta_titles;
        }

        $meta_descriptions = array('en-gb'=>'ENG meta description','pl-pl'=>"POL meta opis");
        $category['meta_descriptions'] = $meta_descriptions;

        $meta_keywords = array('en-gb'=>'ENG meta keyword description','pl-pl'=>"POL meta keyword opis");        
    ...
    public function getCategoriesDescription(&$languages) {
        ...
        foreach ($languages as $language) {
            ...
            $sql = "select * from oc_category left join oc_category_description on oc_category_description.category_id = oc_category.category_id where language_id=".$language_id."";
            $query = $this->db->query( $sql );

            foreach($query->rows as $row) {
                $category_descriptions[$row['category_id']][$language_code] = $row;
            }
        }
    ...
    protected function sortProductOptionValue($product_id) {
        $sql  = "SELECT * from `".DB_PREFIX."product_option_value` left join oc_option_value on oc_product_option_value.option_value_id=oc_option_value.option_value_id where product_id=1";
        $query = $this->db->query($sql);
        print_r($query->rows);        
    }
...
//		}
    ...
    protected function getSimpleProductDescriptions(&$languages) {
        ...
        foreach ($languages as $language) {
            ...
            foreach($query->rows as $row) {
                $product_descriptions[$row['product_id']][$language_code] = array('name'=>$row['name'],'meta_title'=>$row['meta_title'],'meta_description'=>$row['meta_description'],'language_id'=>$language_id,'tag'=>$row['tag'],'meta_keyword'=>$row['meta_keyword']);
            }
        }
...
if (version_compare(VERSION,'3.0','>=')) {
	class ModelExtensionMikran extends ModelToolMikran {
	}
}
