...
class ControllerExtensionPaymentStripe extends Controller {
    ...
    public function stripe_test() {
		...
		// get order info
		$this->load->model('checkout/order');
		$order_info = $this->model_checkout_order->getOrder(43474);
		...
		// handles the XHR request for client side
		$data['action'] = $this->url->link('extension/payment/stripe/confirm_test', '', true);
        $data['fetchkey'] = $this->url->link('extension/payment/stripe/fetchkey', '', true);
        $this->response->setOutput($this->load->view('extension/payment/stripe_test', $data));
    }

    public function fetchkey() {
        $json = array('publicKey' => $this->config->get('payment_stripe_test_public_key'));
        $json_obj = json_encode($json);
        $this->response->setOutput($json_obj);
    }
}